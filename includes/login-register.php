...
function give_process_login_form( $data ) {
	...
	if ( wp_verify_nonce( $data['give_login_nonce'], 'give-login-nonce' ) ) {
		...
		if ( ! $errors ) {
			$redirect = get_permalink($give_options['edit_campaign_page']);
			$args = array(
			...
			if (! empty($u_posts)) {
				foreach ($u_posts as $desired) {
						$redirect = get_permalink($desired->ID);
				}
...
function give_process_register_form( $data ) {
	...
	if ( empty( $errors ) ) {
		...
		give_register_and_login_new_user( array(
			'user_first'	  => $data['give_user_name1'],
			'user_last'		  => $data['give_user_name2'],
...
function redirect_user_campaign() {
	global $give_options;
	$args = array(
		'post_type'		=>	'give_forms',
		'give_forms_category' => 'active',
	...
	$redirect = get_permalink($give_options['edit_campaign_page']);
	if (count($u_posts) != 0) {
		$redirect = get_permalink($u_posts[0]);
	}
	wp_redirect($redirect);
	exit();
}