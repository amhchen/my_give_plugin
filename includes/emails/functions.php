...
function give_admin_email_notice( $payment_id = 0, $payment_data = array() ) {
	...
	$recipient = give_participant_email_notice($payment_id);
...
function give_participant_email_notice( $payment_id = 0) {
	$don = give_get_payment_meta($payment_id);
	$to_form_id = $don['form_id'];
	$form = new Give_Donate_Form($to_form_id);
