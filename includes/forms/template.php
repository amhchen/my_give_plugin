...
//function multiple_selected() {
//add_action( 'give_purchase_form_before_cc_form' , 'donation_split');
//}
function donation_split($form_id, $args = array())
{
	if (!has_term("cause", "give_forms_category", $form_id)) {
		return;
	} else {
		$args = array(
		...
		$list = get_posts($args);
		?>
		<legend class="my-donation-split"><?php echo "Donation Distribution"; ?>
			<span class="give-loading-text"><span
					class="give-loading-animation"></span> <?php _e("Loading...", "give"); ?></span>
		</legend>
		<table id="split-donation-table">
			<tr>
				<td><input class="text-input" id="split-amount-0" name="give-amount-0" type="tel" value="0.00"></td>
				<td>for <select id="split-form-0"><?php
						foreach ($list as $campaign) {
							echo '<option value="' . get_the_title($campaign).'" ';
							if ($campaign->ID == $form_id) {
								echo 'selected';
							}
							echo '>'. get_the_title($campaign) . '</option>';
						}
						?></select></td>
			</tr>
		</table>
		...
		<script>
			function myFunction() {
				var table = document.getElementById("split-donation-table");
				var numrows = table.rows.length;
				var row = table.insertRow(numrows);
				var cell1 = row.insertCell(0);
				var cell2 = row.insertCell(1);
				cell1.innerHTML = '<input class="text-input" id="split-amount-" name="give-amount-" type="tel" placeholder="" value="<?php echo give_get_default_form_amount($form_id); ?>">';
				cell2.innerHTML = 'for <select id="split-form-"><?php
					foreach ($list as $campaign) {
						echo '<option value="' . get_the_title($campaign).'" ';
						if ($campaign->ID == $form_id) {
							echo 'selected';
...
function multiple_recipients($form_id,$args) {
	?><button onclick="enableMultiple(true)">Multiple donations?</button>
	<script>
		function enableMultiple(clicked = false) {
			if(clicked) {<?php add_action('give_after_donation_amount','donation_split'); ?>}
		}
	</script>
	<?php
}

//add_action( 'give_after_donation_amount', 'multiple_recipients', 1, 2 );
/**
...
function give_display_checkout_button( $form_id, $args ) {
	...
	if (has_term('cause','give_forms_category',$form_id)) {
		$display_label = (!empty($display_label_field) ? $display_label_field : __('Donate Now | 捐款', 'give'));
	}
	else {
		$display_label = (!empty($display_label_field) ? $display_label_field: 'Support/贊助 ' . get_the_title($form_id));
	}
	//$output = '<button type="button" class="give-btn give-btn-' . $display_option . '">' . $display_label . '</button>';
	$target_date = get_post_meta( $form_id, '_give_target_date', true );
	if ( intval($target_date) < time() ) {
		$expired_label = 'Campaign Passed';
		$output = '<button type="button" disabled style="background-color:grey" class="give-btn give-btn-' . $display_option . '">' . $expired_label . '</button>';
	}
	else {
		$output = '<button type="button" class="give-btn give-btn-' . $display_option . '" 	>' . $display_label . '</button>';
	}
...
function give_donations_custom_form_fields( $form_id ) {
	...
	?>
	<div id="give-anonymous-wrap">
		<label class="give-label" for="give-anonymous"><?php _e( 'Publish my name on website 在網頁顯示我的名字', 'give' ); ?></label>
		<span class="give-tooltip icon icon-question" data-tooltip="<?php _e( 'Check if it is a anonymous donation.', 'give' ) ?>"></span>
		<br><input class="give-input required" type="checkbox" name="give_anonymous" id="give-anonymous" value="yes" checked><br>
		<br>
	</div>
	<div id="give-message-wrap">
		<label class="give-label" for="give-message"><?php _e( 'Leave message 留言', 'give' ); ?></label>
		<span class="give-tooltip icon icon-question" data-tooltip="<?php _e( 'Please leave your message.', 'give' ) ?>"></span>
		<textarea class="give-textarea" name="give_message" id="give-message"></textarea>
		<br>
		<br>
	</div>
	<div id="give-anonymous-wrap">
		<label class="give-label" for="give-anonymous-to-p"><?php _e( 'Publish my message on website 在網頁顯示我的留言', 'give' ); ?></label>
		<span class="give-tooltip icon icon-question" data-tooltip="<?php _e( 'Check if message is anonymous', 'give' ) ?>"></span>
		<br><input class="give-input required" type="checkbox" name="give_anonymous_message" id="give-anonymous-to-p" value="yes" checked><br>
		<br>
	</div>
	<?php
...
function give_payment_mode_select( $form_id ) {
	...
	<fieldset id="give-payment-mode-select">
		...
		<div id="give-payment-mode-wrap">
			<legend class="give-payment-mode-label"><?php echo apply_filters( 'give_checkout_payment_method_text', 'Payment Method 捐款方式: PayPal/Credit Card 信用卡' ); ?>
			...
			<ul hidden id="give-gateway-radio-list">
...
function give_show_goal_progress( $form_id, $args ) {
	...
	$online      = $form->get_earnings();
	$income		 = $online + get_post_meta($form_id, '_give_offline_money', true);
	...
	$event_date	 = get_post_meta($form_id,'_give_target_date', true);
	...
	$days	  = ceil( ( $event_date - time() ) / (24*60*60) );
	$c_day  = '日';
	$tense	  = (abs($days) == 1)? ' day ':' days ';
	$tense	 .= ($days >=0)? 'to go': 'ago';
	$c_tense  = ($days >=0)? '還有': '前';
	//$days	  = abs($days);
	...
	foreach($terms as $term) {
		...
		if (intval($term->slug) != 0) {
			$year = $term->slug;
		}
	}
	if (has_term('cause','give_forms_category',$form->ID)) { //if ($cause) {
		$c_args = array(
			'post_type' => 'give_forms',
			'numberposts' => -1,
			'tax_query' => array(
		...
		$sub_posts = get_posts($c_args);
		$income = 0;
		foreach($sub_posts as $campaign) {
			$frm = new Give_Donate_Form($campaign->ID);
			$income += $frm->get_earnings();
			$income += get_post_meta($campaign->ID,'_give_offline_money',true);
		}
	}
	/*else if (has_term('group','give_forms_category',$form->ID)) { //if ($group)
		$slug = group;
		$slug .= $form->ID;
		$g_args = array(
			'post_type' => 'give_forms',
	...
	if ( ! empty( $show_text ) ) {
		...
		if ( $goal_format !== 'percentage' ) {
			...
			$output .= sprintf( _x( '%s of %s raised', 'This text displays the amount of income raised compared to the goal.', 'give' ), '<span class="goal-text">' . apply_filters( 'give_goal_amount_raised_output', give_currency_filter( give_format_amount( $income ) ) ) . '</span>', '<span class="goal-text">' . apply_filters( 'give_goal_amount_target_output', give_currency_filter( give_format_amount( $goal ) ) ) ) . ' 捐款進度</span>';

		} elseif ( $goal_format == 'percentage' ) {

			$output .= sprintf( _x( '%s%% funded', 'This text displays the percentage amount of income raised compared to the goal target.', 'give' ), '<span class="give-percentage">' . apply_filters( 'give_goal_amount_funded_percentage_output', round( $progress ) ) . '</span>' ) . ' 捐款進度</span>';
	...
	$output .= '<div>';
	$output .= ($days == 0)? 'Today | 今日': abs($days) . $tense . ' | ';
	if ($days != 0) {$output .= ($days >=0)? $c_tense . $days . $c_day : abs($days) . $c_day . $c_tense;}
	$output .= '</div><br>';
	echo apply_filters( 'give_goal_output', $output );
	do_action('after_show_goal_progress',$form_id,$args);
	return false;
...
add_action('after_show_goal_progress','share_link',9,2);
function share_link($form_id,$args=array()) {
	?><div class="share_link"><p><small>Share this campaign | 分享此籌款網頁</small><br>
		<a href="<?php echo get_permalink($form_id);?>"><small><?php echo get_permalink($form_id);?></small></a></p>
	</div><?php
}

add_action('give_before_donation_levels','tax_receipt_text');
function tax_receipt_text($form_id=0) {
...
function edit_campaign_link($form_id,$args = array()) {
	global $give_options;
	if (get_the_author_meta('ID') == get_current_user_id()&& intval(get_post_meta($form_id,'_give_target_date',true)) > time()){?>
		<a href="<?php echo get_permalink($give_options['edit_campaign_page']);?>">Edit Campaign | 修改我的網頁內容</a>
