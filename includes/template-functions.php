...
function list_of_donors($pid){
	...
	?>
<p><br></p><div><table>
	<caption><br>Sponsors List 贊助名單</caption>
	<tr><td>Date 日期</td><td>Name 名字</td><td>Donation Amount 捐款額</td><td>Message 留言</td></tr>
	<?php
	foreach ($payments as $don) {
		$don_meta = give_get_payment_meta($don->ID);
		if ($don_meta['form_id'] == get_the_ID()) {
	?>
	<tr>
		<td><?php echo give_get_payment_completed_date($don->ID);?></td>
		<td>
	<?php
		...
		echo $cust->name;
	?></td>
		<td><?php echo esc_html( give_currency_filter( give_format_amount( give_get_payment_amount( $don->ID ) ) ) ); ?></td>
		<td><?php echo $don_meta['message'];?></td>
	</tr>
		<?php }} ?>
</table></div><?php } // end at least 1 payment check
}
	add_action('after_after_single_form_summary', 'list_of_donors');