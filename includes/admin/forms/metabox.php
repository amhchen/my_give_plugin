...
function give_single_forms_cmb2_metaboxes( array $meta_boxes ) {
	...
	$offline			   = get_post_meta($post_id,'_give_offline_money',true);
	...
	$meta_boxes['form_field_options'] = apply_filters( 'give_forms_field_options', array(
		...
		'fields'       => apply_filters( 'give_forms_donation_form_metabox_fields', array(
				...
				array(
					'name'		  => 'Target Date',
					'id'		  => $prefix . 'target_date',
					'type'		  => 'text_date_timestamp'
				),
			    // offline earnings
				array(
					'name'		  => 'Offline Earnings',
					'id'		  => $prefix . 'offline_money',
					'type'         => 'text_small',
					'attributes'   => array(
						'placeholder' => give_format_amount( '0.00' ), 
						'value'       => isset( $offline ) ? esc_attr( give_format_amount( $offline ) ) : '',
						'class'       => 'cmb-type-text-small give-money-field',
						)
				)
			)
