...
function give_new_campaign_notification($redirect) {
	...
	$message .= "Thank you for signing up for CCM Walk For Love " . date('Y') . ". ";
	$message .= "Here are your registration details:\n";
	$message .= "多謝你登記中信中心愛履同行2016. 以下是你的登記資料:\n";
	$message .= "Username 戶名: " . $user->user_login . "\n";
	$message .= "Password 密碼: [Password entered during registration 登記密碼]\n";
	$message .= "Campaign Link 籌款網頁: " . '<a href='.$redirect.'>'.$redirect.'<a>' . "\n";
	$message .= "Web ID 網上編號: " . $user->ID . "\n";
	$message .= '<span style="color:red">*Please mark your web ID on your donation envelope'."\n";
	$message .= '*請在你的籌款封上填寫你的網上編號</span>'."\n\n";
	$message .= "CCM Walk for Love " . date('Y') . "\n";
	$message .= "中信中心愛履同行 " . date('Y') . "\n";
	$mail = new Give_Emails();
	$mail->send( $user->user_email, "Welcome!", $message );

}

//add_action( 'give_insert_user', 'give_new_user_notification', 10, 2 );
add_action( 'new_campaign_created', 'give_new_campaign_notification',10,1);
