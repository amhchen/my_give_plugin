<?php
/**
 * Plugin Name: My Give - Donation Plugin
 * Plugin URI: https://givewp.com
 * Description: The most robust, flexible, and intuitive way to accept donations on WordPress.
 * Author: WordImpress
 * Author URI: https://wordimpress.com
 * Version: Prod11 Build
 * Text Domain: give
...
if ( ! class_exists( 'Give' ) ) :
	...
	final class Give {
		...
		private function includes() {
			...
			require_once GIVE_PLUGIN_DIR . 'includes/forms/original-widget.php';
...
//For disabling admin bar for non-admin users
...
function wti_loginout_menu_link( $items, $args ) {
	if ($args->theme_location == 'primary') {
		if (is_user_logged_in()) {
			$items .= '<li><a href="'. wp_logout_url(home_url()) .'">Log Out <br/> 登出</a></li>';
		} else {
			$items .= '<li><a href="'. wp_login_url() .'">Log In <br/> 登入</a></li>';
		}
	}
	return $items;
}

add_filter('login_url','my_login');
function my_login($login_url,$redirect="",$force_reauth="") {
	global $give_options;
	return get_permalink($give_options['login_page']);
}